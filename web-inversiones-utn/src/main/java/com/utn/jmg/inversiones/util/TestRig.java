...
//	}
	...
	public void process() throws Exception {
		...
		try {
			ClaseLoaderG ddd = new ClaseLoaderG ();
			lexerClass = ddd.findClass(lexerName).asSubclass(Lexer.class);
			//lexerClass = Class.forName(lexerName, true, cl).asSubclass(Lexer.class);
	...
	protected void process(Lexer lexer, Class<? extends Parser> parserClass, Parser parser, CharStream input)
			throws IOException, IllegalAccessException, InvocationTargetException, PrintException, Exception {
		...
		try {
			...
			if (printTree) {
				String resultado = tree.toStringTree(parser);
				if (parser.getNumberOfSyntaxErrors() > 0) {

					throw new Exception("Error en Formula.");
				}
				// System.out.println(tree.toStringTree(parser));
				// if(resultado.contains("token recognition error")){
				// throw new IndicadorException("Error en Formula.");
				// }
			}
			// if (gui) {
